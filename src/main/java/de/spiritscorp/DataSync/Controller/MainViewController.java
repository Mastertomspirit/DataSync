...
public class MainViewController implements ViewController {
	...
	public void handleApplicationShutdown() {
		...
		if (confirmation.showAndWait().orElse(ButtonType.CANCEL) == ButtonType.OK) {
			...
			// 1. Tear down the JavaFX UI framework layer immediately so the window closes for the user
			Platform.exit();

			// 2. Spawn a dedicated user thread to process the graceful shutdown without blocking the UI thread
			final Thread manualShutdownWorker = new Thread(() -> {
				try {
					// User manual exit allows for a more generous, dynamic grace period
//					executeCoreShutdownSequence(true);
				} finally {
			...
			// Must be a user thread (false) to guarantee the JVM waits for its completion
			manualShutdownWorker.setDaemon(false);
			manualShutdownWorker.start();
		}
	}

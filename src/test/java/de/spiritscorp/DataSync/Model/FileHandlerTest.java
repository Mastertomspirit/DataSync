...
class FileHandlerTest {
	...
	final void testFindDuplicates() {
		...
		Path[] expectedDuplicates = helper.getDuplicates();
		assertEquals(
		...
		for(Path path : expectedDuplicates) {
			true(duplicateMap.containsKey(path),
	...
	final void testGetSyncFiles() throws IOException {
		...
		ArrayList<Map<Path, FileAttributes>> expectedValues = 
		...
		ArrayList<Map<Path,FileAttributes>> syncFiles = fileHandler.getSyncFiles(
			sourceMap, destMap, 
		...
		for(Entry<Path, FileAttributes> entry : expectedCopySource.entrySet()) {
			FileAttributes actual = actualCopySource.get(entry.getKey());
			assertNotNull(
				actual,
		...
		for(Entry<Path, FileAttributes> entry : expectedCopyDest.entrySet()) {
			FileAttributes actual = actualCopyDest.get(entry.getKey());
			assertNotNull(
				actual,
		...
		for(Entry<Path, FileAttributes> entry : expectedDel.entrySet()) {
			FileAttributes actual = actualDel.get(entry.getKey());
			assertNotNull(
				actual,
	...
	final void testEdgeCaseIdenticalTimestamps() {
		...
		int initialSourceSize = sourceMap.size();
		int initialDestSize = destMap.size();
	...
	final void testEdgeCaseEmptyMaps() {
		...
		// Execute
		ArrayList<Map<Path,FileAttributes>> result = fileHandler.getSyncFiles(
			sourceMap, destMap, 
			testPath.resolve("source"), 
			testPath.resolve("dest"), 
			syncMap);

...
### 1. Multi-Source Ingestion Framework
...
### 2. Directory Content Flattening (`subdir` Configuration)
...
### 3. Integrated Isolation Trashbin (Exclusive to Backup Mode)
...
### 4. Automation vs. Manual Interactive Analysis (`Auto-Sync` & `Auto-Delete`)
...
### 5. Structured Machine-Readable JSON Logging
...
### 6. Dynamic Configuration Routing (`--configPath` Bootstrap Flag)
The application lifecycle can be dynamically re-routed during execution initialization by utilizing the new `--configPath <path>` CLI bootstrap argument. Passing this specific operational flag overrides the standard application directory layout:
...
### III. Deep Scan (Cryptographic Verification Backup)
...
### IV. Duplicate Scan (Asynchronous Duplicity Analysis)

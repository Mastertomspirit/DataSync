...
//		START BUTTON
		if (e.getSource() == view.getStartButton()) {
			if (!helper.isScanRun() && pref.getSourcePath() != null && pref.getDestPath() != null) {
				...
				if (pref.getDeepScan() == ScanType.DUBLICATE_SCAN)
					new Thread(() -> helper.startDuplicateScan(view)).start();
				else if (pref.getDeepScan() == ScanType.SYNCHRONIZE)
					new Thread(() -> helper.startSyncronize(view)).start();
				else
					new Thread(() -> helper.startBackup(view)).start();
			} else {
...
//		SAVE ALL PREFERENCES
		if (e.getSource() == view.getPreference()) {
			if (pref.savePrefs())
				view.setTextArea("Einstellungen erfolgreich gespeichert");
			else
				view.setTextArea("Einstellungen konnten nicht gespeichert werden");
		}
